...
/**
 * Hook for managing Stripe payments and subscriptions in web environments
 */
export const useStripeWeb = () => {
    const [loadingPayment, setLoadingPayment] = useState(false);

    const context = useContext(FirewandContext);
    if (!context) {
        throw new Error('FirewandContext must be used within a FirewandProvider');
    }
    ...
    const newCheckoutSession = async (sessionConfig: SessionConfig): Promise<void> => {
        ...
        try {
            // Get the current URL without query params for success/cancel redirects
            ...
            // Open the Stripe checkout URL (web only)
